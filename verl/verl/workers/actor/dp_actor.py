...
from verl.utils.tide_reward import tide_scores
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_distillation_reward(self, data: DataProto) -> DataProto:
        ...
        tide_keep_neg = data.meta_info.get("tide_keep_neg", 0.2)
        tide_keep_pos = data.meta_info.get("tide_keep_pos", 0.2)
        tide_deficit_lambda = data.meta_info.get("tide_deficit_lambda", 1.0)
        micro_batch_size = data.meta_info["micro_batch_size"]
        temperature = data.meta_info["temperature"]
        use_dynamic_bsz = data.meta_info["use_dynamic_bsz"]
        ...
        S_on_T = None
        if strategy in ["only_tch", "intersection", "union", "union-intersection", "tide"]:
        ...
        if strategy == "only_stu":
            ...
            # TIDE: candidate slot 0 is the sampled token (excess branch,
            # bounded suppression through the Hellinger-shaped reward), slots
            # 1..K are the teacher top-K (deficit branch, analytic
            # cross-entropy). Positions outside both quantile-selected active
            # sets receive exactly zero advantage on every candidate slot.
            sampled_ids = data.batch["responses"].to(device)
            sampled_student_logp = data.batch["old_log_probs"].to(device)
            sampled_teacher_logp = data.batch["teacher_log_probs"].to(device)
            response_mask = data.batch["response_mask"].to(device)

            tide = tide_scores(
                sampled_student_logp=sampled_student_logp,
                sampled_teacher_logp=sampled_teacher_logp,
                teacher_topk_logp=T_logp,
                student_on_teacher_logp=S_on_T,
                response_mask=response_mask,
                keep_neg=tide_keep_neg,
                keep_pos=tide_keep_pos,
                deficit_lambda=tide_deficit_lambda,
            )
            rm_scores = torch.cat(
                [tide["excess_scores"].unsqueeze(-1), tide["deficit_scores"]], dim=-1
            )
            # Candidate ids aligned with the rm_scores slots; update_policy
            # gathers student log-probs at these ids, so the gradient flows
            # through the sampled token AND every teacher top-K candidate.
            res_tensors["union_top_k_ids"] = torch.cat(
                [sampled_ids.unsqueeze(-1).to(T_ids.dtype), T_ids], dim=-1
            )
            res_tensors["union_top_k_log_probs"] = torch.cat(
                [sampled_student_logp.unsqueeze(-1), S_on_T], dim=-1
            )

            valid_count = response_mask.sum().clamp(min=1).float()
            print(
                "[tide] "
                f"excess_frac={(tide['excess_mask'].sum() / valid_count).item():.4f} "
                f"deficit_frac={(tide['deficit_mask'].sum() / valid_count).item():.4f} "
                f"tau_neg={tide['tau_neg']:.4f} tau_pos={tide['tau_pos']:.4f} "
                f"residual_mean={(tide['sampled_residual'].sum() / valid_count).item():.4f} "
                f"deficit_stat_mean={(tide['deficit_stat'].sum() / valid_count).item():.4f}"
            )
