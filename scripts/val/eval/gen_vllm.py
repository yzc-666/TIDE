...
# --------------------------------------------------------------------------- #
DATA_DIR = os.environ.get("EVAL_DATA_DIR", "../../../datasets/test_data")
# Either point EVAL_MODEL_FOLDER at a directory of checkpoints (evaluated
# highest-step first) or give a single model with EVAL_MODEL_PATH.
MODEL_FOLDER = os.environ.get("EVAL_MODEL_FOLDER", "")
...
# Active model list.
MODEL_NAMES = MODEL_NAMES_CANDIDATES or [p for p in [os.environ.get("EVAL_MODEL_PATH")] if p]
if not MODEL_NAMES:
    raise SystemExit(
        "No model to evaluate. Set EVAL_MODEL_PATH=<hf model dir>, "
        "or EVAL_MODEL_FOLDER=<dir of checkpoints>."
    )
...
def main():
    ...
    for model_name in MODEL_NAMES:
        ...
        OUT_DIR = Path(f"eval_outputs/{model_name.split('/')[-1]}")
