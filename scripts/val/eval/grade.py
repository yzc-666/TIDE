...
# Name of the run to grade: the sub-directory gen_vllm.py wrote under eval_outputs/,
# which is the basename of the evaluated model.
NAME     = os.environ.get("EVAL_RUN_NAME", "")
EVAL_DIR = Path(os.environ.get("EVAL_OUTPUT_DIR", "eval_outputs")) / NAME
OUTPUT_FILE = EVAL_DIR / "grading_results.json"
# Only used with --enable_model_verifier.
MODEL_NAME = os.environ.get("EVAL_VERIFIER_PATH", "CompassVerifier-3B")

if not NAME:
    raise SystemExit("Set EVAL_RUN_NAME=<sub-directory under eval_outputs/> to grade a run.")

# Global variables to be initialized if needed
vllm_model = None
model_tokenizer = None
sampling_params = None
# Optional: only used to report response length in tokens instead of characters.
_length_tokenizer_path = os.environ.get("EVAL_LENGTH_TOKENIZER", "")
length_tokenizer = (
    AutoTokenizer.from_pretrained(_length_tokenizer_path) if _length_tokenizer_path else None
)
