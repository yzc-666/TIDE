...
# source /path/to/conda/bin/activate verl
...
export TOP_K_STRATEGY=${TOP_K_STRATEGY:-"tide"} # "tide" (ours) / "only_stu" / "only_tch" / "intersection" / "union" / "union-intersection"
export REWARD_WEIGHT_MODE=${REWARD_WEIGHT_MODE:-"student_p"} # "student_p" or "teacher_p" or "none"
# TIDE (TOP_K_STRATEGY=tide): quantile keep-rates + deficit branch weight
export TIDE_KEEP_NEG=${TIDE_KEEP_NEG:-0.2} # rho^-: fraction of tokens kept in the excess (suppression) active set
export TIDE_KEEP_POS=${TIDE_KEEP_POS:-0.2} # rho^+: fraction of positions kept in the deficit (teacher top-K CE) active set
export TIDE_DEFICIT_LAMBDA=${TIDE_DEFICIT_LAMBDA:-1.0} # lambda: weight of the deficit branch
# export LR=${LR:-1e-6}
...
export REWARD_DTYPE=${REWARD_DTYPE:-$MODEL_DTYPE} # teacher/reward model dtype; defaults to MODEL_DTYPE (fp32). Set REWARD_DTYPE=bfloat16 for large teachers to avoid OOM.
...
export MODEL_ROOT=${MODEL_ROOT:-hf_models}

# TODO:
# export ACTOR_MODEL_PATH=${MODEL_ROOT}/Qwen3-1.7B-Base
# export ACTOR_MODEL_PATH=${MODEL_ROOT}/Qwen3-8B-Base
# export ACTOR_MODEL_PATH=${MODEL_ROOT}/Qwen3.5-2B-Base
# export ACTOR_MODEL_PATH=${MODEL_ROOT}/lllyx/Qwen3-4B-Base-GRPO
...
# export REWARD_MODEL_PATH=${MODEL_ROOT}/Qwen3-8B-Base
# export REWARD_MODEL_PATH=${MODEL_ROOT}/lllyx/Qwen3-4B-Base-GRPO
# export REWARD_MODEL_PATH=${MODEL_ROOT}/Skywork/Skywork-OR1-Math-7B
...
# Root for checkpoints and experiment metadata. Override this to place all
# run artifacts on another filesystem, e.g. PROJECT_PATH=/path/to/opd.
export PROJECT_PATH=${PROJECT_PATH:-checkpoint}
export PARALLEL_SIZE=1
# Fixed experiment name (no timestamp) so preempted jobs can resume from the same dir.
# Override with: EXPERIMENT_NAME=xxx bash on_policy_distillation.sh
# Start a brand-new run with: NEW_RUN=1 bash on_policy_distillation.sh
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-${ADV_ESTIMATOR}_${TRAIN_DATASET_NAME}_${ACTOR_MODEL_NAME}_${REWARD_MODEL_NAME}_${MAX_RESP_LENGTH}-T_${TEMPERATURE}-Tch_${TEACHER_TEMPERATURE}-n_${N_RESPONSES}-mbs_${MINI_BATCH_SIZE}-topk_${LOG_PROB_TOP_K}-topk_strategy_${TOP_K_STRATEGY}-rw_${REWARD_WEIGHT_MODE}}
if [ "${NEW_RUN:-0}" = "1" ]; then
    export EXPERIMENT_NAME=${EXPERIMENT_NAME}-$(date +%Y-%m-%d_%H-%M-%S)
fi
export CKPT_PATH=${CKPT_PATH:-${PROJECT_PATH}/${EXPERIMENT_NAME}}
export VALIDATION_DATA_DIR=${VALIDATION_DATA_DIR:-${PROJECT_PATH}/validation_log/${EXPERIMENT_NAME}}
export ROLLOUT_DATA_DIR=${ROLLOUT_DATA_DIR:-null}
# Hydra otherwise dumps its job config into ./outputs/<date>/<time> next to the
# repo; keep it on the same filesystem as the rest of the run artifacts.
export HYDRA_RUN_DIR=${HYDRA_RUN_DIR:-${PROJECT_PATH}/hydra_outputs/${EXPERIMENT_NAME}/$(date +%Y-%m-%d_%H-%M-%S)}
export RESUME_MODE=${RESUME_MODE:-auto}  # auto | disable | resume_path
export RESUME_FROM_PATH=${RESUME_FROM_PATH:-}  # used when RESUME_MODE=resume_path
export SAVE_FREQ=${SAVE_FREQ:-20}  # save more often on preemptible nodes
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.tide_keep_neg=$TIDE_KEEP_NEG \
    +actor_rollout_ref.rollout.tide_keep_pos=$TIDE_KEEP_POS \
    +actor_rollout_ref.rollout.tide_deficit_lambda=$TIDE_DEFICIT_LAMBDA \
    ...
    +reward_model.model.dtype=$REWARD_DTYPE \
